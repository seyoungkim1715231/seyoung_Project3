...
public class update5 {//영화 UPDATE--테스트완 성공 !!
	...
	public void ud1() throws ClassNotFoundException {
		...
		try {
			...
			String sql = "INSERT INTO MOVIE (ARTICLE_TITLE, RE_DT, DT, ETC_CHAR11, LINK_URL)"
			           +" VALUES (?,?,?,?,?)";

			String sql1="SELECT count(*) as COUNT FROM MOVIE WHERE ARTICLE_TITLE=? AND DT=?";//행개수로 탐색할까?
			ResultSet rs=null;
			...
			for (int page = 1; page < 2; page++) {//상위 30개만 검토 
				...
				StringBuilder urlBuilder = new StringBuilder(
						"https://open.assembly.go.kr/portal/openapi/nfzxkpetatunooatq"); /* URL */
				...
				JSONArray parse_body = (JSONArray) obj.get("nfzxkpetatunooatq");// 이부분 수정필요함

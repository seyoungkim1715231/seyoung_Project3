...
public class update2 {//전체회의 UPDATE--테스트완 성공 !!
	...
	public void ud1() throws ClassNotFoundException {
		...
		try {
			...
			String sql = "INSERT INTO COMMSCHE_MAIN (MEETING_DATE, MEETING_TIME, SESS, TITLE, COMMITTEE_NAME, LINK_URL2, "
			           +"UNIT_CD, UNIT_NM, HR_DEPT_CD)"
			           +" VALUES (?,?,?,?,?,?,?,?,?)";
			String sql1="SELECT count(*) as COUNT FROM COMMSCHE_MAIN WHERE TITLE=? AND MEETING_DATE=?";//행개수로 탐색할까?
			ResultSet rs=null;
			...
			for (int page = 1; page < 2; page++) {//상위 30개만 검토 
				...
				StringBuilder urlBuilder = new StringBuilder(
						"https://open.assembly.go.kr/portal/openapi/nttmdfdcaakvibdar"); /* URL */
				...
				JSONArray parse_body = (JSONArray) obj.get("nttmdfdcaakvibdar");// 이부분 수정필요함
